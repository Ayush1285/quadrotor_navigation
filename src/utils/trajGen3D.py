...
def get_lemniscate_waypoints(t,n):
    waypoints_t = np.linspace(t, t + 2*np.pi, n)
    x = 5*np.sin(waypoints_t)
    y = 5*np.sin(2*waypoints_t)
    z = waypoints_t
...
def generate_trajectory(t, v, waypoints, coeff_x, coeff_y, coeff_z):
    ...
    return DesiredState(pos, vel, acc, 0, 0)
...
def line_waypoints(t,n):
    waypoints_t = np.linspace(t, t + 4, n)
    x = 2*waypoints_t
    y = 2*waypoints_t
    z = 0*waypoints_t
...
def trajasarray(t,dt,v,waypoints,coeff_x, coeff_y, coeff_z):
    position = np.array([[0,0,0]])
    velocity = np.array([[0,0,0]])
    accln = np.array([[0,0,0]])

    while True:

        if np.linalg.norm(waypoints[-1,:]-position[-1,:]) < 0.1:
            break
        
        des_states = generate_trajectory(t, v, waypoints, coeff_x, coeff_y, coeff_z)
        
        position = np.r_[position,np.array([[des_states[0][0],des_states[0][1],des_states[0][2]]])]
        velocity = np.r_[position,np.array([[des_states[1][0],des_states[1][1],des_states[1][2]]])]
        accln = np.r_[position,np.array([[des_states[2][0],des_states[2][1],des_states[2][2]]])]
        t = t + dt

    return position[1:,:],velocity[1:,:],accln[1:,:]
